...
router.post('/register', async(req,res)=>{
    ...
    try {
        if(user) {
            ...
            res.status(201).json({message: `Thank you for registering, ${user.full_name}!`, userInfo: AddUser, token:token})
        } else {
    ...
    Users.getUserBy({ email })
        ...
        .then(async (user) => {
            if (user && bcrypt.compareSync(password, user.password)) {
                ...
                const token = signToken(user)
                res.status(200).json({ message: `User login successful ${user.full_name}`, userInfo: GetUser, token: token })
            } else {
